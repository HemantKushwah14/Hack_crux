...
def fetch_emails(service, max_results=5):
    ...
    for msg in messages:
        ...
        # Check if email is received after app start time
        if received_at >= app_start_time:
            body = msg_data.get('snippet', '')
            formatted_received_at = format_time(received_at)
            email_data.append({'id': msg['id'], 'subject': subject, 'sender': sender, 'body': body, 'received_at': formatted_received_at})
...
def index():
    ...
    return render_template('index.html', emails=email_responses)
